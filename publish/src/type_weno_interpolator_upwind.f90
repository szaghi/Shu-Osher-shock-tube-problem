...
contains
  ...
  character(len=23)                            :: dummy            !< Dummy string.
  ...
  write(dummy, "(I15)") 2*self%S - 1
  string = string//'  Provide a formal order of accuracy equals to: '//trim(adjustl(dummy))//nl
  write(dummy, "(I15)") self%S
  string = string//'  Use '//trim(adjustl(dummy))//' stencils composed by '//trim(adjustl(dummy))//' values'//nl
  write(dummy, "(E23.15)") self%eps
  string = string//'  The eps value used for avoiding division by zero is '//trim(adjustl(dummy))//nl
